...
export class SeriesListComponent implements OnInit, OnDestroy {
  ...
  private readonly API_URL = 'http://localhost:3000/api/series';
  ...
  private initForm(): void {
    this.seriesForm = this.fb.group({
      ...
      series: ['', [Validators.required, Validators.minLength(2), Validators.maxLength(4), Validators.pattern(/^[A-Za-z0-9]+$/)]],
  ...
  loadEntities(): void {
    ...
    if (this.filterSeries.trim()) url += `&series=${this.filterSeries.trim().toUpperCase()}`;
    if (this.filterDocumentType) url += `&documentType=${this.filterDocumentType}`;

    this.http.get<{ status: string; data: { data: SeriesItem[]; total: number } }>(url).subscribe({
      next: (res: any) => {
  ...
  resetFilters(): void { this.filterSeries = ''; this.filterDocumentType = ''; this.currentPage = 1; }
  ...
  openCreateModal(): void {
    ...
    this.seriesForm.reset({ documentType: 'FACTURA', series: '', currentNumber: 0, description: '' });
    this.showModal = true;
    this.cdr.detectChanges();
  }
  ...
  onFormSubmit(): void {
    ...
    if (this.isEditing && this.selectedSeriesId) {
      this.http.put(`${this.API_URL}/${this.selectedSeriesId}`, payload).subscribe({
  ...
  onDelete(id: number): void {
    if (confirm('¿Está seguro de dar de baja este talonario de series? No podrá reutilizarse la numeración.')) {
  ...
  private handleError(err: any): void {
    this.isLoading = false;
    this.formErrorMessage = err?.error?.message || 'Error al procesar el talonario en MySQL.';
    this.cdr.detectChanges();
  }
